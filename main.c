...
int main() {
    L_Char_LinkList L = NULL;
    CharListTailInsert(&L);
    L_Char_Node *p = ListGetCharElem(L, 2);
    printCharList(L);
    ListDeleteCharNode(p);
//    printf("%d\n", ListInsertCharNode(L, &q, 2));
    printf("%d\n", CharListLen(L));
    printCharList(L);
}
//#include <stdio.h>
...
//    _ftprintf(stderr, TEXT("Error number %lx.\n"), GetLastError());
